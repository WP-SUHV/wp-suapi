...
class WP_SUAPI_Shortcode_Manager
{
    ...
    public function __construct()
    {
        ...
        add_shortcode('wp-suapi-fixturetable-team', (array($this, 'fixturesTableTeam')));
        add_shortcode('wp-suapi-fixturetable-club', (array($this, 'fixturesTableClub')));
    }
     ...
     */
    public function fixturesTableTeam($atts)
    {
        $a = shortcode_atts(array(
            'year' => 0,
            'team' => 0,
        ...
        $apiHandler = WP_SUAPI_Helper::GET_INITIALIZED_API_HANDLER();
        $apiHandler->setYearForQuery($a['year']);
        $fixtures = $apiHandler->getGamesForTeam(new Team($a['team'], ''));

        $args = array('fixtures' => $fixtures);
    ...
    /*
     * Shortcode function for game fixtures tables for club
     * Attributes:
     *  year: Year for query (e.g. 2014)
     *  club: swiss unihockey Club ID
     *  type: Table Type
     *        |-1: Date, Home Team, Away Team, Location, Result
     */
    public function fixturesTableClub($atts)
    {
        $a = shortcode_atts(array(
            'year' => 0,
            'club' => 0,
        ...
        if ($a['year'] == 0 || $a['club'] == 0)
            return "";

        $apiHandler = WP_SUAPI_Helper::GET_INITIALIZED_API_HANDLER();
        $apiHandler->setYearForQuery($a['year']);
        $fixtures = $apiHandler->getGamesForClub(new Club($a['club'], ''));
